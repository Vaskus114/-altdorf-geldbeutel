(() => {
  ...
  const combatTab = character => {
    ...
    return `${sheetBox("Trefferzonen","Rüstung",`<button class="mini-button" id="edit-resources">Manuell</button>`,armourBody,"combat-armour-box")}${sheetBox("Belastung","Getragene Last","",loadBody,"combat-load-box")}${sheetBox("Ausrüstung","Angelegt & griffbereit",`<button class="mini-button" id="add-equipment">＋</button>`,equipmentBody,"combat-equipment-box")}${sheetBox("Magische Effekte","Aktiv","",effectBody,"combat-effects-box")}`;
  };
  const skillsTab = character => {
    ...
    return `${sheetBox(`Grundregelwerk · ${RULES.skills.length} Skills`,"Skills",`<button class="mini-button" id="add-skill">＋</button>`,`${hint}<div class="simple-list skills-list">${character.sheet.skills.length?character.sheet.skills.map(skill=>`<article><div><strong>${escapeHtml(skill.name)}</strong><small>${skill.source==="core"?"GRW · ":"frei · "}${escapeHtml(skill.attribute||"frei")}${skill.note?` · ${escapeHtml(skill.note)}`:""}</small></div><div class="item-actions"><button class="mini-button" data-edit-skill="${skill.id}">✎</button><button class="mini-button danger" data-delete-skill="${skill.id}">×</button></div></article>`).join(""):`<div class="empty compact"><h3>Noch keine Skills</h3><p>Skills können direkt aus der Karriere-Ansicht gelernt werden.</p></div>`}</div>`,"skills-box")}`;
  };

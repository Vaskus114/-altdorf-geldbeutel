(() => {
  ...
  const statCard = (character,stat) => {
    const data=character.sheet.stats[stat.key]; const total=statValue(character,stat.key);
    return `<article class="stat-card"><small>${escapeHtml(stat.key)}</small><strong>${total}</strong><span>${num(data.base)} ${num(data.advance)>=0?"+":""}${num(data.advance)} ${num(data.manual)>=0?"+":""}${num(data.manual)}</span></article>`;
  };
  const resourceCards = character => {
    const r=character.sheet.resources;
    return `<div class="resource-grid"><article><small>Wunden</small><strong>${r.currentWounds}/${woundsMax(character)}</strong></article><article><small>Schicksal</small><strong>${r.fateCurrent}/${r.fateMax}</strong></article><article><small>Magiepunkte</small><strong>${r.magicCurrent}/${r.magicMax}</strong></article><article><small>EP</small><strong>${r.xpAvailable}</strong><span>${r.xpTotal} gesamt</span></article></div>`;
  };
  const profileTab = character => {
    const i=character.sheet.identity;
    return `<div class="sheet-section"><div class="section-title"><div><span class="eyebrow">Person</span><h3>${escapeHtml(character.name)}</h3></div><button class="mini-button" id="edit-profile">Bearbeiten</button></div>
      <div class="profile-facts"><span><small>Karriere</small><strong>${escapeHtml(character.career)}</strong></span><span><small>Volk</small><strong>${escapeHtml(i.race||"—")}</strong></span><span><small>Alter</small><strong>${escapeHtml(i.age||"—")}</strong></span><span><small>Gesinnung</small><strong>${escapeHtml(i.alignment||"—")}</strong></span><span><small>Geburtsort</small><strong>${escapeHtml(i.birthplace||"—")}</strong></span><span><small>Frühere Karrieren</small><strong>${escapeHtml(i.previousCareers||"—")}</strong></span></div>
      ${i.marks?`<p class="character-note"><strong>Besondere Merkmale:</strong> ${escapeHtml(i.marks)}</p>`:""}${i.notes?`<p class="character-note">${escapeHtml(i.notes)}</p>`:""}
    </div>
    <div class="sheet-section"><div class="section-title"><div><span class="eyebrow">Profilwerte</span><h3>Charakteristika</h3></div><button class="mini-button" id="edit-stats">Bearbeiten</button></div><div class="stat-grid">${RULES.characteristics.map(stat=>statCard(character,stat)).join("")}</div></div>
    <div class="sheet-section"><div class="section-title"><div><span class="eyebrow">Aktuell</span><h3>Ressourcen</h3></div><button class="mini-button" id="edit-resources">Bearbeiten</button></div>${resourceCards(character)}</div>`;
  };
  const careerTab = character => {
    ...
    return `<div class="sheet-section"><div class="section-title"><div><span class="eyebrow">Grundregelwerk</span><h3>${escapeHtml(character.career)}</h3></div><button class="mini-button" id="choose-career">Karriere wählen</button></div><p class="character-note">${c.type==="custom"?"Freie Karriere":"Core-Karriere · "+(c.type==="basic"?"Basic":"Advanced")}. Advance Schemes bleiben vollständig editierbar; die App führt nur Werte aus, die du im Schema bestätigst.</p></div>
      <div class="sheet-section"><div class="section-title"><div><span class="eyebrow">Advance Scheme</span><h3>Steigerungen</h3></div><button class="mini-button" id="edit-career-scheme">Schema bearbeiten</button></div><div class="scheme-grid">${scheme.map(key=>{const max=num(c.scheme?.[key],0),cur=num(character.sheet.stats[key]?.advance,0),step=["S","T","W","A"].includes(key)?1:10;const can=max>0&&cur+step<=max&&available>=num(RULES.rules?.advanceCost,100);return `<article class="scheme-card"><small>${key}</small><strong>${cur} / +${max}</strong>${max?`<button class="mini-button" data-buy-advance="${key}" ${can?"":"disabled"}>+${step} · 100 EP</button>`:`<span>—</span>`}</article>`}).join("")}</div>${!c.freeAdvanceUsed?`<p class="hint">Der bei der Charaktererschaffung vorgesehene freie Advance kann einmal ohne EP-Kosten genutzt werden.</p><button class="secondary-button" id="free-advance">Freien Anfangs-Advance nutzen</button>`:""}</div>
      <div class="sheet-section"><div class="section-title"><div><span class="eyebrow">Erfahrung</span><h3>${available} EP verfügbar</h3></div></div><p class="character-note">Ein Charakteristik-Advance kostet 100 EP. Neue Skills einer Karriere kosten 100 EP. Prozentwerte steigen dabei um 10; S, T, W und A um 1.</p></div>`;
  };
  const armourCard = (location,part) => `<article class="armour-zone"><small>${locationLabel(location)}</small><strong>${armourDisplay(part)}</strong><span>${part.effects?`Effekt ${signed(part.effects)} · `:""}${part.manual?`Manuell ${signed(part.manual)} · `:""}${part.leatherSuppressed?"Leder unter Metall ohne Zusatzschutz":"AP"}</span></article>`;
  ...
  const combatTab = character => {
    const armour=armourBreakdown(character); const warnings=armourWarnings(character); const equipment=character.inventory.filter(item=>item.type==="armor"||item.type==="weapon"); const cap=carryingCapacity(character);
    return `<div class="sheet-section"><div class="section-title"><div><span class="eyebrow">Trefferzonen</span><h3>Rüstung</h3></div><button class="mini-button" id="edit-resources">Manuell</button></div><div class="armour-grid">${RULES.locations.map(location=>armourCard(location,armour[location])).join("")}</div>${warnings.length?`<div class="rules-warning"><strong>Rüstungshinweis</strong>${warnings.map(w=>`<p>${escapeHtml(w)}</p>`).join("")}</div>`:""}</div>
    <div class="sheet-section"><div class="section-title"><div><span class="eyebrow">Belastung</span><h3>Getragene Last</h3></div><small>${bodyEnc(character)}${cap?` / ${cap}`:""} ENC</small></div>${cap&&bodyEnc(character)>cap?`<div class="rules-warning"><strong>Überladen</strong><p>Die getragene Last liegt über der aus Stärke × 100 abgeleiteten Traglast.</p></div>`:""}</div>
    <div class="sheet-section"><div class="section-title"><div><span class="eyebrow">Ausrüstung</span><h3>Angelegt &amp; griffbereit</h3></div><button class="mini-button" id="add-equipment">＋</button></div><div class="equipment-list">${equipment.length?equipment.map(item=>`<article class="equipment-row ${item.equipped?"equipped":""}"><div><strong>${escapeHtml(item.name)}</strong><small>${item.type==="armor"?`${item.equipped?"angelegt":"nicht angelegt"} · ${item.armourMaterial==="leather"?"0/1 Leder":"Rüstung"}`:weaponSummary(character,item)}</small></div><div class="item-actions"><button class="mini-button" data-equip="${item.id}">${item.equipped?"Ablegen":"Anlegen"}</button><button class="mini-button" data-edit-item="${item.id}">✎</button></div></article>`).join(""):`<div class="empty compact"><h3>Noch keine Kampf-Ausrüstung</h3><p>Rüstung oder Waffen aus dem Inventar können hier automatisch wirken.</p></div>`}</div></div>
    ${character.sheet.spells.some(spell=>spell.active&&spell.armourBonus)?`<div class="sheet-section"><div class="section-title"><div><span class="eyebrow">Magische Effekte</span><h3>Aktiv</h3></div></div>${character.sheet.spells.filter(spell=>spell.active&&spell.armourBonus).map(spell=>`<p class="character-note">${escapeHtml(spell.name)}: ${signed(spell.armourBonus)} AP auf alle Trefferzonen</p>`).join("")}</div>`:""}`;
  };
  const skillsTab = character => `<div class="sheet-section"><div class="section-title"><div><span class="eyebrow">Grundregelwerk · ${RULES.skills.length} Skills</span><h3>Skills</h3></div><button class="mini-button" id="add-skill">＋</button></div><div class="simple-list">${character.sheet.skills.length?character.sheet.skills.map(skill=>`<article><div><strong>${escapeHtml(skill.name)}</strong><small>${skill.source==="core"?"GRW · ":"frei · "}${escapeHtml(skill.attribute||"frei")} ${skill.note?`· ${escapeHtml(skill.note)}`:""}</small></div><div class="item-actions"><button class="mini-button" data-edit-skill="${skill.id}">✎</button><button class="mini-button danger" data-delete-skill="${skill.id}">×</button></div></article>`).join(""):`<div class="empty compact"><h3>Noch keine Skills</h3><p>Über + kannst du aus allen Core-Skills wählen oder einen freien Eintrag anlegen.</p></div>`}</div></div>`;
  ...
  const characterSheet = (tab="profile") => {
    ...
    const layer=modal(`<section class="sheet character-sheet"><div class="handle"></div><div class="heading"><div><span class="eyebrow">WFRP 1. Edition</span><h2>${escapeHtml(character.name)}</h2></div><button class="close" aria-label="Schließen">×</button></div>${sheetTabs(tab)}${content}</section>`);
    $(".close",layer).addEventListener("click",()=>layer.remove());

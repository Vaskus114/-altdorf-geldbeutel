(() => {
  ...
  const sanitizeItemEffect = effect => ({
    id:effect?.id || id(),
    name:String(effect?.name || "Effekt").slice(0,120),
    kind:["weapon","armour","magic","trait","other"].includes(effect?.kind)?effect.kind:"trait",
    description:String(effect?.description || "").slice(0,3000),
    enabled:effect?.enabled !== false,
    profileBonuses:sanitizeProfileBonuses(effect?.profileBonuses,"")
  });
  const effectKindLabel = kind => ({weapon:"Waffenfähigkeit",armour:"Rüstungseffekt",magic:"Magisch",trait:"Besondere Fähigkeit",other:"Sonstiger Effekt"}[kind] || "Effekt");
  const sanitizeHerb = herb => ({
    otherNomenclatures:String(herb?.otherNomenclatures || "").slice(0,1500),
    availability:String(herb?.availability || "").slice(0,120),
    locality:String(herb?.locality || "").slice(0,300),
    season:String(herb?.season || "").slice(0,160)
  });
  const sanitizeItem = item => ({
    ...
    description:String(item?.description || item?.rules || "").slice(0,6000),
    note:String(item?.note || "").slice(0,1000),
    type:["generic","armor","weapon","herb"].includes(item?.type)?item.type:"generic",
    presetId:String(item?.presetId||"").slice(0,80),
    equipped:Boolean(item?.equipped),
    herb:sanitizeHerb(item?.herb),
    ...
    effects:Array.isArray(item?.effects)?item.effects.slice(0,30).map(sanitizeItemEffect):[]
  });
  ...
  const defaultSheet = () => ({
    ...
    mental:{insanityPoints:0,disorders:""},
  ...
  const sanitizeSheet = sheet => {
    ...
    const mental=sheet?.mental||{};
    base.mental={insanityPoints:Math.max(0,num(mental.insanityPoints,0)),disorders:String(mental.disorders||"").slice(0,6000)};
  ...
  const skillProfileModifier = (character,key) => {
    ...
    (character.sheet.skills||[]).forEach(skill=>{
      ...
      if(!amount) return;
      total+=amount;
      details.push({name:skill.name,amount,source:"Skill/Talent"});
    });
    return {total,details};
  };
  const activeEffectProfileModifier = (character,key) => {
    const details=[];
    let total=0;
    (character.sheet.spells||[]).filter(spell=>spell.active).forEach(spell=>{
      const amount=num(spell?.profileBonuses?.[key],0);
      if(!amount) return;
      total+=amount;
      details.push({name:spell.name,amount,source:"Zauber"});
    });
    (character.inventory||[]).filter(item=>item.equipped&&(item.type==="weapon"||item.type==="armor")).forEach(item=>{
      (item.effects||[]).filter(effect=>effect.enabled).forEach(effect=>{
        const amount=num(effect?.profileBonuses?.[key],0);
        if(!amount) return;
        total+=amount;
        details.push({name:`${effect.name} · ${item.name}`,amount,source:"Ausrüstungseffekt"});
      });
    });
    return {total,details};
  };
  const profileModifier = (character,key) => {
    const skill=skillProfileModifier(character,key),effect=activeEffectProfileModifier(character,key);
    return {total:skill.total+effect.total,details:[...skill.details,...effect.details]};
  };
  const startStatValue = (character,key) => {
    const stat=character.sheet.stats[key]||{base:0,advance:0,purchased:0,manual:0};
    return num(stat.base)+profileModifier(character,key).total;
  };
  ...
  const advancePurchaseState = (character,key,schemeMaximum=null) => {
    const stat=character.sheet.stats?.[key]||{};
    const max=Math.max(0,num(schemeMaximum==null?stat.advance:schemeMaximum,0));
    const step=advanceStep(key);
    const bought=purchasedAdvance(character,key);
    const remaining=Math.max(0,max-bought);
    return {max,step,bought,remaining,canBuy:max>0&&remaining>=step,reached:max>0&&bought>=max};
  };
  const woundsMax = character => Math.max(0,statValue(character,"W"));
  const isDwarf = character => /\b(?:zwerg|zwerge|dwarf|dwarfs)\b/i.test(String(character?.sheet?.identity?.race||""));
  const carryingMultiplier = character => isDwarf(character)?200:100;
  const carryingCapacity = character => Math.max(0,statValue(character,"S")*carryingMultiplier(character));
  ...
  const movementState = character => {
    const base=Math.max(0,statValue(character,"M"));
    const cap=carryingCapacity(character);
    const carried=bodyEnc(character);
    const over=Math.max(0,carried-cap);
    const penalty=over>0?Math.ceil(over/50):0;
    return {base,current:Math.max(0,base-penalty),penalty,over,cap,carried,multiplier:carryingMultiplier(character)};
  };
  ...
  const allItemPresets = () => [...(RULES.armourPresets||[]),...(RULES.weaponPresets||[]),...(RULES.equipmentPresets||[]),...(RULES.herbPresets||[])];
  ...
  const enableTouchFormFocus = root => {
    ...
    const refocusFromEvent=event=>{
      ...
      if(!control||!root.contains(control)||control.disabled||control.readOnly)return;
      // Safari/iPadOS can leave a field visually focused without opening the software
      // keyboard inside fixed, scrollable sheets. Re-focusing directly from the user's
      // touch gesture restores the expected keyboard behaviour, including controls that
      // are added to a form dynamically after the dialog has opened.
      try{control.focus({preventScroll:true})}catch{control.focus()}
      requestAnimationFrame(()=>{
        if(document.activeElement===control){
          try{control.scrollIntoView({block:"center",inline:"nearest",behavior:"auto"})}catch{}
        }
      });
    };
    if(window.PointerEvent){
      root.addEventListener("pointerup",event=>{if(event.pointerType==="touch")refocusFromEvent(event)},{passive:true,capture:true});
    }else{
      root.addEventListener("touchend",refocusFromEvent,{passive:true,capture:true});
    }
    if(isIOSLike())root.addEventListener("click",event=>{
      const control=event.target?.closest?.(selector);
      if(control&&document.activeElement!==control)refocusFromEvent(event);
    },true);
  };
  const modal = content => {
  ...
  const statProfileTable = character => {
    ...
    const startCell=stat=>{
      const mod=profileModifier(character,stat.key);
      const title=mod.details.map(item=>`${item.source}: ${item.name} ${signed(item.amount)}`).join(" · ");
      return `<span class="start-stat-value"${title?` title="${escapeHtml(title)}"`:""}>${startStatValue(character,stat.key)}${mod.total?`<sup class="profile-skill-star" aria-label="Profilbonus">*</sup>`:""}</span>`;
    };
    const movement=movementState(character);
    const currentCell=stat=>{
      if(stat.key!=="M"||!movement.penalty) return `<span class="current-stat-value">${statValue(character,stat.key)}</span>`;
      return `<span class="current-stat-value movement-encumbered" title="Überladen um ${movement.over} ENC: −${movement.penalty} Movement"><strong>${movement.current}</strong><small>Original ${movement.base} · Last −${movement.penalty}</small></span>`;
    };
    const compactCards=RULES.characteristics.map(stat=>{
      ...
      const title=mod.details.map(item=>`${item.source}: ${item.name} ${signed(item.amount)}`).join(" · ");
      const current=stat.key==="M"&&movement.penalty?`<span class="movement-encumbered"><strong>${movement.current}</strong><small>Original ${movement.base}<br>Last −${movement.penalty}</small></span>`:statValue(character,stat.key);
      return `<article class="profile-stat-card"><header><strong>${escapeHtml(stat.key)}</strong><small>${escapeHtml(stat.label)}</small></header><dl><div><dt>Start</dt><dd${title?` title="${escapeHtml(title)}"`:""}>${start}${mod.total?`<sup class="profile-skill-star" aria-label="Profilbonus">*</sup>`:""}</dd></div><div><dt>Advanced</dt><dd>${advanced?signed(advanced):"0"}</dd></div><div class="current"><dt>Current</dt><dd>${current}</dd></div></dl></article>`;
    }).join("");
    ...
    const skillMods=(RULES.characteristics||[]).map(stat=>{const mod=profileModifier(character,stat.key);return mod.total?`${stat.key} ${signed(mod.total)} (${mod.details.map(x=>`${x.source}: ${x.name}`).join(", ")})`:""}).filter(Boolean).join(" · ");
    return `<div class="profile-table-wrap"><table class="profile-table"><thead><tr><th scope="col">Profil</th>${header}</tr></thead><tbody>${row("Start",startCell,"profile-start-row")}${row("Advanced",stat=>{const value=num(character.sheet.stats[stat.key]?.advance,0);return value?signed(value):"0"},"profile-advanced-row")}${row("Current",currentCell,"profile-current-row")}</tbody></table></div><div class="profile-compact-grid" aria-label="Profilwerte">${compactCards}</div><div class="profile-calculation-note"><span><strong>Gekauft:</strong> ${bought||"noch keine Advances"}</span>${skillMods?`<span><strong>* Start inkl. Profilbonus:</strong> ${escapeHtml(skillMods)}</span>`:""}${movement.penalty?`<span><strong>Belastung:</strong> M Original ${movement.base} → ${movement.current} (−${movement.penalty})</span>`:""}</div>`;
  };
  const resourceCards = character => {
    const r=character.sheet.resources,mental=character.sheet.mental||{insanityPoints:0,disorders:""};
    const disorders=String(mental.disorders||"").trim();
    return `<div class="resource-grid"><article><small>Wunden</small><strong>${r.currentWounds}/${woundsMax(character)}</strong><span>aktuell / max</span></article><article><small>Schicksal</small><strong>${r.fateCurrent}/${r.fateMax}</strong><span>aktuell / max</span></article><article><small>Magiepunkte</small><strong>${r.magicCurrent}/${r.magicMax}</strong><span>aktuell / max</span></article><article><small>Erfahrung</small><strong>${r.xpAvailable}</strong><span>${r.xpTotal} gesamt</span></article><article><small>Insanity Points</small><strong>${mental.insanityPoints}</strong><span>aktuell</span></article><article class="resource-disorders-card"><small>Disorders</small><strong>${disorders?escapeHtml(disorders):"—"}</strong><span>psychische Störungen</span></article></div>`;
  };
  const profileTab = character => {
    const i=character.sheet.identity,current=currentCareerRecord(character),history=careerHistory(character).map(entry=>entry.name).filter(Boolean),portrait=character.sheet.portrait||{dataUrl:"",name:""};
    ...
    const portraitCard=portrait.dataUrl?`<aside class="character-portrait-card"><button type="button" class="portrait-image-button" id="portrait-open" aria-label="Portrait von ${escapeHtml(character.name)} anzeigen"><img src="${portrait.dataUrl}" alt="Portrait von ${escapeHtml(character.name)}"></button><div class="portrait-actions"><button class="mini-button" type="button" id="portrait-open-secondary">Anzeigen</button><button class="mini-button" type="button" id="portrait-upload-button">Ändern</button><button class="mini-button danger" type="button" id="portrait-remove">Entfernen</button></div></aside>`:`<aside class="character-portrait-card empty-portrait"><div class="portrait-placeholder" aria-hidden="true">♙</div><strong>Charakterportrait</strong><small>JPG, PNG oder WebP</small><button class="mini-button" type="button" id="portrait-upload-button">Portrait hochladen</button></aside>`;
    ...
    const body=`<input class="hidden" id="portrait-file" type="file" accept="image/*"><div class="profile-character-layout">${portraitCard}<div class="profile-character-details">${meta}${grid}</div></div>`;
    return `${sheetBox("Charakter",escapeHtml(character.name),`<button class="mini-button" id="edit-profile">Bearbeiten</button>`,body,"identity-box")}${sheetBox("Charakteristika","Profilwerte",`<button class="mini-button" id="edit-stats">Bearbeiten</button>`,statProfileTable(character),"stats-box")}${sheetBox("Ressourcen","Wunden, Schicksal, Magie & Insanity",`<button class="mini-button" id="edit-resources">Bearbeiten</button>`,resourceCards(character),"resources-box")}`;
  };
  const careerTab = character => {
    ...
    const schemeBody=`<div class="scheme-grid">${(RULES.advanceableCharacteristics||[]).map(key=>{const max=num(scheme[key],0),progress=advancePurchaseState(character,key,max),bought=progress.bought,step=progress.step,remaining=progress.remaining,cost=num(RULES.rules?.advanceCost,100),withinScheme=progress.canBuy,hasXp=available>=cost,canBuy=withinScheme&&hasXp,canRefund=bought>=step;const buyReason=!withinScheme?"Maximum des Schemas bereits erreicht":!hasXp?`${cost-available} EP fehlen`:"Kauf möglich";const progressText=max?`gekauft ${bought?signed(bought):"0"} / Maximum ${signed(max)}${remaining>0?` · noch ${signed(remaining)}`:" · vollständig"}`:"kein Advance";return `<article class="scheme-card"><small>${key}</small><strong>${max?signed(max):"0"}</strong><span>Schema · ${progressText}</span>${max?`<div class="scheme-actions"><button type="button" class="mini-button advance-buy ${canBuy?"":"is-blocked"}" data-buy-advance="${key}" aria-disabled="${canBuy?"false":"true"}" ${withinScheme?"":"disabled"} title="${escapeHtml(buyReason)}">+${step} kaufen · ${cost} EP</button><button type="button" class="mini-button advance-refund" data-refund-advance="${key}" ${canRefund?"":"disabled"}>−${step} zurück · +${cost} EP</button>${canBuy?"":`<small class="advance-action-state">${escapeHtml(buyReason)}</small>`}</div>`:`<span>—</span>`}</article>`}).join("")}</div><p class="hint"><strong>Advanced</strong> ist das Gesamtmaximum gegenüber dem Starterprofil und <strong>nicht kumulativ zwischen Karrieren</strong>. Beispiel: Wurde <strong>A +1</strong> bereits in einer früheren Karriere gekauft und die neue Karriere erlaubt <strong>A +2</strong>, bleibt nur noch <strong>ein</strong> weiterer A-Advance offen. Danach ist das Schema ausgeschöpft. Skill-, Talent- und Magieboni zählen dabei nicht als gekaufte Advances. Mit <strong>zurück</strong> kannst du einen versehentlich gekauften Schritt wieder entfernen; die ${num(RULES.rules?.advanceCost,100)} EP werden erstattet.</p>`;
  ...
  const combatTab = character => {
    const armour=armourBreakdown(character),warnings=armourWarnings(character),equipment=character.inventory,movement=movementState(character),cap=movement.cap,carried=movement.carried;
    ...
    const loadBody=`<div class="resource-grid combat-strip encumbrance-strip"><article><small>Getragene Last</small><strong>${carried}</strong><span>ENC am Körper</span></article><article><small>Traglast</small><strong>${cap||0}</strong><span>Stärke × ${movement.multiplier}${movement.multiplier===200?" · Zwerg":""}</span></article><article class="movement-card ${movement.penalty?"penalized":""}"><small>Movement</small><strong>${movement.current}</strong><span>${movement.penalty?`Original ${movement.base} · −${movement.penalty}`:`Original ${movement.base} · keine Laststrafe`}</span></article><article><small>Rüstungs-Punkte</small><strong>${physicalArmourPoints(character)}</strong><span>relevant für Magie</span></article><article><small>Magie-Malus</small><strong>+${spellArmourSurcharge(character)}</strong><span>MP pro Zauber</span></article></div>${movement.penalty?`<div class="rules-warning encumbrance-warning"><strong>Überladen · Movement −${movement.penalty}</strong><p>${movement.over} ENC über der Traglast. Nach dem Grundregelwerk verliert der Charakter für je 50 ENC oder angefangene 50 ENC über dem Maximum 1 Punkt seiner Movement Allowance: M ${movement.base} → ${movement.current}.</p></div>`:""}`;
    const equipmentBody=`<div class="equipment-list">${equipment.length?equipment.map(item=>{
      const effectNames=(item.effects||[]).map(effect=>effect.name).filter(Boolean);
      const effectInfo=effectNames.length?` · Effekte: ${escapeHtml(effectNames.join(", "))}`:"";
      const details=item.type==="armor"
        ? `${item.equipped?"angelegt":"nicht angelegt"} · ${item.armourMaterial==="leather"?"0/1 Leder":item.armourMaterial==="gambeson"?"0/1 Gambeson":"Rüstung"} · ${itemEnc(item)} ENC${effectInfo}`
        : item.type==="weapon"
          ? `${weaponSummary(character,item)} · ${itemEnc(item)} ENC${effectInfo}`
      ...
      return `<article class="equipment-row equipment-row-with-description ${item.equipped?"equipped":""}"><div class="equipment-row-main"><div><strong>${escapeHtml(item.name)}${item.quantity>1?` ×${item.quantity}`:""}</strong><small>${details}</small></div><div class="item-actions">${equipButton}<button class="mini-button" data-edit-item="${item.id}">✎</button></div></div>${itemDescriptionDetails(item,"equipment-description")}</article>`;
    ...
    const activeSpells=character.sheet.spells.filter(spell=>spell.active);
    const activeItemEffects=character.inventory.filter(item=>item.equipped&&(item.type==="weapon"||item.type==="armor")).flatMap(item=>(item.effects||[]).filter(effect=>effect.enabled).map(effect=>({item,effect})));
    const effectCount=activeSpells.length+activeItemEffects.length;
    const effectBonusSummary = bonuses => RULES.characteristics.map(stat=>{const amount=num(bonuses?.[stat.key],0);return amount?`${stat.key} ${signed(amount)}`:""}).filter(Boolean).join(" · ");
    const spellEffects=activeSpells.map(spell=>{const profile=effectBonusSummary(spell.profileBonuses);return `<article class="active-effect-card spell-effect"><div class="active-effect-head"><span class="effect-kind">Zauber</span><strong>${escapeHtml(spell.name)}</strong></div>${spell.armourBonus?`<p><b>Rüstungswirkung:</b> ${signed(spell.armourBonus)} AP auf alle Trefferzonen.</p>`:""}${profile?`<p><b>Profilwirkung:</b> ${escapeHtml(profile)}.</p>`:""}${spell.description?`<p>${escapeHtml(spell.description)}</p>`:""}</article>`}).join("");
    const equipmentEffects=activeItemEffects.map(({item,effect})=>{const profile=effectBonusSummary(effect.profileBonuses);return `<article class="active-effect-card item-effect"><div class="active-effect-head"><span class="effect-kind">${escapeHtml(effectKindLabel(effect.kind))}</span><strong>${escapeHtml(effect.name)}</strong></div><small>Quelle: ${escapeHtml(item.name)}</small>${profile?`<p><b>Profilwirkung:</b> ${escapeHtml(profile)}.</p>`:""}${effect.description?`<p>${escapeHtml(effect.description)}</p>`:`<p class="hint">Keine Beschreibung hinterlegt.</p>`}</article>`}).join("");
    const effectBody=effectCount?`<div class="active-effects-list">${spellEffects}${equipmentEffects}</div>`:`<p class="hint">Derzeit sind keine Zauber oder Ausrüstungseffekte aktiv. Effekte von Waffen und Rüstungen erscheinen hier, sobald der Gegenstand angelegt/griffbereit ist.</p>`;
    return `${sheetBox("Trefferzonen","Rüstung",`<button class="mini-button" id="edit-resources">Manuell</button>`,armourBody,"combat-armour-box")}${sheetBox("Belastung","Getragene Last","",loadBody,"combat-load-box")}${sheetBox("Ausrüstung","Am Körper",`<button class="mini-button" id="add-equipment">＋</button>`,equipmentBody,"combat-equipment-box")}${sheetBox("Magische Effekte",`${effectCount} aktiv`,"",effectBody,"combat-effects-box")}`;
  };
  ...
  const magicTab = character => {
    ...
    const book=`<div class="spell-list">${character.sheet.spells.length?character.sheet.spells.map(spell=>{const total=Math.max(0,num(spell.magicPoints))+surcharge;const profile=RULES.characteristics.map(stat=>{const amount=num(spell.profileBonuses?.[stat.key],0);return amount?`${stat.key} ${signed(amount)}`:""}).filter(Boolean).join(" · ");return `<article class="spell-card ${spell.active?"active":""}"><div class="spell-title"><div><strong>${escapeHtml(spell.name)}</strong><small>${escapeHtml(spell.school||"frei")} · Grad ${escapeHtml(spell.level||"—")} · ${spell.costVerified?`${spell.magicPoints} MP + ${surcharge} Rüstung = ${total} MP`:"MP-Kosten noch nicht bestätigt"} · ${escapeHtml(spell.range||"Reichweite frei")}</small></div><span>${spell.active?"AKTIV":""}</span></div>${spell.duration?`<p><b>Dauer:</b> ${escapeHtml(spell.duration)}</p>`:""}${spell.ingredients?`<p><b>Zutaten:</b> ${escapeHtml(spell.ingredients)}</p>`:""}${spell.description?`<details class="rule-description spell-description"><summary>Beschreibung &amp; Wirkung</summary><p>${escapeHtml(spell.description)}</p></details>`:""}${spell.armourBonus?`<p><b>Automatik:</b> ${signed(spell.armourBonus)} AP auf alle Trefferzonen, solange aktiv.</p>`:""}${profile?`<p><b>Profilbonus aktiv:</b> ${escapeHtml(profile)}.</p>`:""}<div class="storage-toolbar"><button class="secondary-button" data-cast-spell="${spell.id}" ${spell.costVerified?"":"disabled"}>Wirken (${total} MP)</button><button class="secondary-button" data-toggle-spell="${spell.id}">${spell.active?"Deaktivieren":"Aktivieren"}</button></div><div class="item-actions right"><button class="mini-button" data-edit-spell="${spell.id}">✎ Bearbeiten</button><button class="mini-button danger" data-delete-spell="${spell.id}">×</button></div></article>`}).join(""):`<div class="empty compact"><h3>Noch keine Zauber</h3><p>Über + kannst du Core-Zauber übernehmen.</p></div>`}</div>`;
  ...
  const openPortrait = character => {
    ...
    const viewer=modal(`<section class="sheet portrait-viewer"><div class="handle"></div><div class="heading"><div><span class="eyebrow">${escapeHtml(character.name)}</span><h2>Charakterportrait</h2></div><button class="close" aria-label="Schließen">×</button></div><div class="portrait-viewer-frame"><img src="${portrait.dataUrl}" alt="Portrait von ${escapeHtml(character.name)}"></div>${portrait.name?`<p class="portrait-file-name">${escapeHtml(portrait.name)}</p>`:""}</section>`);
    viewer.classList.add("high");$(".close",viewer).addEventListener("click",()=>viewer.remove());
  };
  const savePortrait = async (file,layer,tab) => {
    const character=active(),previous=clone(character.sheet.portrait||{dataUrl:"",name:""});
    try{
  ...
  const characterSheet = (tab="profile") => {
    ...
    $("#portrait-upload-button",layer)?.addEventListener("click",()=>$("#portrait-file",layer)?.click());
  ...
  const buyAdvance = (key,previous) => {
    ...
    const stat=character.sheet.stats[key];
    if(!stat) return;
    const progress=advancePurchaseState(character,key,stat.advance);
    const {max,step,bought}=progress;
    const cost=num(RULES.rules?.advanceCost,100);
    if(!progress.canBuy){alert(max?`Das aktuelle Advance Scheme erlaubt für ${key} insgesamt nur ${signed(max)}. Davon wurden bereits ${bought?signed(bought):"0"} gekauft.`:`Das aktuelle Advance Scheme erlaubt für ${key} keine Steigerung.`);return}
  ...
  const refundAdvance = (key,previous) => {
    const character=active();
    const career=currentCareerRecord(character);
    syncAdvancedFromCareerScheme(character,career,false);
    const stat=character.sheet.stats[key];
    ...
    const bought=purchasedAdvance(character,key);
    const cost=num(RULES.rules?.advanceCost,100);
    if(bought<step){alert(`Für ${key} ist kein gekaufter Advance vorhanden, der zurückgenommen werden kann.`);return}
    if(!confirm(`Den letzten gekauften ${key}-Advance (${step>1?`+${step}`:`+${step}`}) zurücknehmen und ${cost} EP erstatten?`))return;
    stat.purchased=Math.max(0,bought-step);
    character.sheet.resources.xpAvailable+=cost;
    persist();
    previous.remove();
    render();
  ...
  const editStats = (previous,returnTab) => {
    ...
    const cards=RULES.characteristics.map(stat=>{
      const mod=profileModifier(character,stat.key);
      const modTitle=mod.details.map(item=>`${item.source}: ${item.name} ${signed(item.amount)}`).join(" · ");
    ...
    const layer=modal(`<form class="sheet form stat-profile-form"><div class="handle"></div><div class="heading"><div><span class="eyebrow">Profilwerte</span><h2>Charakteristika</h2></div><button type="button" class="close">×</button></div><div class="profile-edit-grid">${cards}</div><div class="profile-calculation-note"><span><strong>Gekaufte Advances:</strong> ${bought||"noch keine"}</span><span><strong>*</strong> kennzeichnet einen in Start eingerechneten Profilbonus aus Skill/Talent oder aktivem Effekt.</span></div><p class="hint"><strong>Start</strong> = Wert aus der Charaktererschaffung plus Profilboni aus Skills/Talenten und aktuell aktiven Effekten. <strong>Advanced</strong> = maximal kaufbare Werte aus dem Scheme der aktuellen Karriere und frei editierbar. <strong>Current</strong> = effektiver Start + tatsächlich gekaufte Advances. Advanced selbst wird nicht auf Current addiert. Eine Überlastung verändert den gespeicherten M-Wert nicht; im Charakterbogen wird daraus regelkonform die reduzierte Movement Allowance berechnet und der Originalwert daneben gezeigt.</p><button class="full-button">Werte speichern</button></form>`);
    ...
    const refreshCurrent=()=>RULES.characteristics.forEach(stat=>{
      ...
      const modifier=profileModifier(character,stat.key).total;
  ...
  const editResources = (previous,returnTab) => {
    const character=active(),r=character.sheet.resources,m=character.sheet.armourManual,mental=character.sheet.mental||{insanityPoints:0,disorders:""};
    const layer=modal(`<form class="sheet form"><div class="handle"></div><div class="heading"><div><span class="eyebrow">Aktuelle Werte</span><h2>Ressourcen, Insanity &amp; manuelle AP</h2></div><button type="button" class="close">×</button></div><div class="form-grid two"><label>Aktuelle Wunden<input name="currentWounds" type="number" value="${r.currentWounds}"></label><label>Max. Wunden (aus W)<input value="${woundsMax(character)}" disabled></label><label>Schicksal aktuell<input name="fateCurrent" type="number" value="${r.fateCurrent}"></label><label>Schicksal max.<input name="fateMax" type="number" value="${r.fateMax}"></label><label>Magiepunkte aktuell<input name="magicCurrent" type="number" value="${r.magicCurrent}"></label><label>Magiepunkte max.<input name="magicMax" type="number" value="${r.magicMax}"></label><label>EP verfügbar<input name="xpAvailable" type="number" value="${r.xpAvailable}"></label><label>EP gesamt<input name="xpTotal" type="number" value="${r.xpTotal}"></label><label>Insanity Points<input name="insanityPoints" type="number" min="0" step="1" value="${mental.insanityPoints}"></label><label>Magie-Rüstungs-AP (Override)<input name="magicArmourOverride" type="number" min="0" value="${character.sheet.magic.armourPointOverride}"></label></div><label>Disorders<textarea name="disorders" rows="5" placeholder="z. B. Phobie, Zwang, weitere psychische Störungen">${escapeHtml(mental.disorders)}</textarea><small>Freies Feld für einen oder mehrere Disorders; Zeilenumbrüche sind möglich.</small></label><span class="eyebrow">Manueller Rüstungsmodifikator</span><div class="form-grid two">${RULES.locations.map(location=>`<label>${locationLabel(location)}<input name="armour-${location}" type="number" min="-20" max="20" value="${m[location]||0}"></label>`).join("")}</div><p class="hint">Die manuellen AP werden zusätzlich zu angelegter Rüstung und aktiven Zaubereffekten verrechnet.</p><button class="full-button">Speichern</button></form>`);
    layer.classList.add("high"); $(".close",layer).addEventListener("click",()=>layer.remove()); $("form",layer).addEventListener("submit",event=>{event.preventDefault();const f=event.currentTarget.elements;Object.keys(r).forEach(key=>r[key]=num(f[key].value));character.sheet.mental={insanityPoints:Math.max(0,num(f.insanityPoints.value,0)),disorders:String(f.disorders.value||"").trim().slice(0,6000)};character.sheet.magic.armourPointOverride=f.magicArmourOverride.value===""?"":Math.max(0,num(f.magicArmourOverride.value));RULES.locations.forEach(location=>character.sheet.armourManual[location]=num(f[`armour-${location}`].value));persist();layer.remove();previous.remove();render();characterSheet(returnTab)});
  };
  ...
  const castSpell = (spellId,previous) => {
    ...
    if(spell.armourBonus || RULES.characteristics.some(stat=>num(spell.profileBonuses?.[stat.key],0)!==0)) spell.active=true;
  ...
  const itemMeta = item => {
    ...
    if(item.type==="herb"){
      parts.push("Kraut");
      if(item.herb?.availability)parts.push(`Verfügbarkeit ${escapeHtml(item.herb.availability)}`);
      if(item.herb?.season)parts.push(`Saison ${escapeHtml(item.herb.season)}`);
    }
    if(item.effects?.length)parts.push(`${item.effects.length} Effekt${item.effects.length===1?"":"e"}`);
    if(item.note)parts.push(escapeHtml(item.note));
    return parts.join(" · ");
  };
  const itemDescriptionDetails = (item,extraClass="") => {
    const text=String(item?.description||"").trim();
    const herb=item?.type==="herb"?sanitizeHerb(item?.herb):null;
    const hasHerb=Boolean(herb&&(herb.otherNomenclatures||herb.availability||herb.locality||herb.season));
    if(!text&&!hasHerb)return "";
    const herbFacts=hasHerb?`<div class="herb-facts">
      ${herb.availability?`<div><small>Verfügbarkeit</small><strong>${escapeHtml(herb.availability)}</strong></div>`:""}
      ${herb.locality?`<div><small>Vorkommen</small><strong>${escapeHtml(herb.locality)}</strong></div>`:""}
      ${herb.season?`<div><small>Saison</small><strong>${escapeHtml(herb.season)}</strong></div>`:""}
    </div>${herb.otherNomenclatures?`<p class="herb-aliases"><strong>Weitere Bezeichnungen:</strong> ${escapeHtml(herb.otherNomenclatures)}</p>`:""}`:"";
    return `<details class="rule-description item-rule-description ${extraClass}"><summary>${item?.type==="herb"?"Kräuterdaten &amp; Regeln":"Beschreibung &amp; Regeln"}</summary>${herbFacts}${text?`<p class="item-rule-text">${escapeHtml(text)}</p>`:""}</details>`;
  };
  ...
  const locationSheet = locationId => {
    ...
    const layer=modal(`<section class="sheet"><div class="handle"></div><div class="heading"><div><span class="eyebrow">${isBody?"Persönliches Inventar":escapeHtml(storageTypeLabel(storage.type))}</span><h2>${escapeHtml(title)}</h2></div><button class="close" aria-label="Schließen">×</button></div>
      ...
      <div class="item-list">${items.length?items.map(item=>`<article class="item-row item-row-with-description"><div class="item-row-main"><span><strong>${escapeHtml(item.name)}${item.quantity>1?` ×${item.quantity}`:""}</strong><small>${itemMeta(item)}</small></span><span class="item-actions">${isBody&&(item.type==="armor"||item.type==="weapon")?`<button class="mini-button" data-equip-item="${item.id}" title="${item.equipped?"Ablegen":"Anlegen"}">${item.equipped?"✓":"○"}</button>`:""}<button class="mini-button" data-edit-item="${item.id}" title="Bearbeiten">✎</button><button class="mini-button" data-move="${item.id}" title="Verschieben">↔</button><button class="mini-button danger" data-delete-item="${item.id}" title="Löschen">×</button></span></div>${itemDescriptionDetails(item)}</article>`).join(""):`<div class="empty"><h3>Hier liegt noch nichts</h3><p>Füge einen Gegenstand hinzu oder verschiebe Beute hierher.</p></div>`}</div>
  ...
  const presetOptions = selected => `<option value="">— frei / kein Preset —</option><optgroup label="Rüstung">${RULES.armourPresets.map(p=>presetOption(p,selected)).join("")}</optgroup><optgroup label="Waffen">${RULES.weaponPresets.map(p=>presetOption(p,selected)).join("")}</optgroup>${(RULES.herbPresets||[]).length?`<optgroup label="Kräuter · Identify Plants">${RULES.herbPresets.map(p=>presetOption(p,selected)).join("")}</optgroup>`:""}${equipmentPresetGroups().map(group=>`<optgroup label="${escapeHtml(group.label)}">${group.items.map(p=>presetOption(p,selected)).join("")}</optgroup>`).join("")}`;
  const itemEffectEditorRow = effect => {
    const e=sanitizeItemEffect(effect||{name:"",kind:"trait",description:"",enabled:true});
    const bonusFields=RULES.characteristics.map(stat=>`<label>${escapeHtml(stat.key)} Bonus<input data-effect-bonus="${stat.key}" type="number" step="1" value="${num(e.profileBonuses?.[stat.key],0)}"></label>`).join("");
    return `<article class="item-effect-editor-row" data-effect-row><div class="item-effect-editor-head"><label>Effekt / Fähigkeit<input data-effect-name value="${escapeHtml(e.name==="Effekt"?"":e.name)}" placeholder="z. B. Armour Piercing"></label><label>Art<select data-effect-kind><option value="weapon" ${e.kind==="weapon"?"selected":""}>Waffenfähigkeit</option><option value="armour" ${e.kind==="armour"?"selected":""}>Rüstungseffekt</option><option value="magic" ${e.kind==="magic"?"selected":""}>Magisch</option><option value="trait" ${e.kind==="trait"?"selected":""}>Besondere Fähigkeit</option><option value="other" ${e.kind==="other"?"selected":""}>Sonstiger Effekt</option></select></label></div><label>Beschreibung / Regelwirkung<textarea data-effect-description placeholder="Regelwirkung oder kurze Notiz">${escapeHtml(e.description)}</textarea></label><details class="profile-bonus-editor item-effect-profile-bonus"><summary>Profilbonus solange Effekt aktiv</summary><p class="hint">Optional, z. B. I +10. Der Bonus gilt nur, solange dieser Effekt aktiviert und die Waffe/Rüstung angelegt bzw. griffbereit ist.</p><div class="form-grid two">${bonusFields}</div></details><div class="item-effect-editor-footer"><label class="check-line compact-check"><input data-effect-enabled type="checkbox" ${e.enabled?"checked":""}><span>Aktiv, solange der Gegenstand angelegt / griffbereit ist</span></label><button type="button" class="mini-button danger" data-remove-item-effect>Effekt entfernen</button></div></article>`;
  };
  const itemEditor = (locationId,itemId,previous,returnContext={locationId}) => {
    ...
    const layer=modal(`<form class="sheet form equipment-form"><div class="handle"></div><div class="heading"><div><span class="eyebrow">Inventar &amp; Charakterbogen</span><h2>${existing?"Gegenstand bearbeiten":"Gegenstand hinzufügen"}</h2></div><button type="button" class="close">×</button></div><label>Vorlage<select name="presetId">${presetOptions(item.presetId)}</select></label><div class="form-grid two"><label>Name<input name="name" required value="${escapeHtml(item.name)}" placeholder="z. B. Kettenmantel"></label><label>Art<select name="type"><option value="generic" ${item.type==="generic"?"selected":""}>Allgemein</option><option value="herb" ${item.type==="herb"?"selected":""}>Kräuter</option><option value="armor" ${item.type==="armor"?"selected":""}>Rüstung / Schild</option><option value="weapon" ${item.type==="weapon"?"selected":""}>Waffe</option></select></label><label>Anzahl<input name="quantity" type="number" min="1" max="9999" value="${item.quantity}"></label><label>ENC je Stück<input name="enc" type="number" min="0" max="99999" step="1" value="${item.enc}"></label></div><span class="eyebrow">Wert je Stück</span>${moneyFields(item.value)}${locationId==="body"?`<label class="check-line" id="equipped-line"><input name="equipped" type="checkbox" ${item.equipped?"checked":""}><span>Aktiv angelegt / griffbereit</span></label>`:""}
      <section id="herb-fields" class="nested-fields"><div class="section-title"><div><span class="eyebrow">Kräuterkunde</span><h3>Kräuterdaten</h3></div></div><div class="form-grid two"><label>Verfügbarkeit<input name="herbAvailability" value="${escapeHtml(item.herb?.availability||"")}"></label><label>Saison<input name="herbSeason" value="${escapeHtml(item.herb?.season||"")}"></label></div><label>Vorkommen / Locality<input name="herbLocality" value="${escapeHtml(item.herb?.locality||"")}"></label><label>Weitere Bezeichnungen<textarea name="herbOtherNomenclatures" rows="3">${escapeHtml(item.herb?.otherNomenclatures||"")}</textarea></label><p class="hint">Die 21 Kräuter-Presets stammen aus deinem Dokument „Identify_Plants.docx“. Dort sind keine ENC- oder Preiswerte angegeben; diese Felder bleiben deshalb frei editierbar.</p></section>
      ...
      <section id="weapon-fields" class="nested-fields"><div class="section-title"><div><span class="eyebrow">Automatik</span><h3>Waffenwerte</h3></div></div><div class="form-grid two"><label>Modus<select name="weaponMode"><option value="melee" ${w.mode==="melee"?"selected":""}>Nahkampf</option><option value="missile" ${w.mode==="missile"?"selected":""}>Fernkampf</option></select></label><label>Spezialwaffen-Fertigkeit<input name="weaponSkill" value="${escapeHtml(w.skill)}"></label><label>Initiative-Mod.<input name="weaponInitiative" value="${escapeHtml(w.initiative)}"></label><label>Treffer-Mod.<input name="weaponToHit" value="${escapeHtml(w.toHit)}"></label><label>Schaden-Mod.<input name="weaponDamage" value="${escapeHtml(w.damage)}"></label><label>Parade-Mod.<input name="weaponParry" value="${escapeHtml(w.parry)}"></label><label>Effektive Stärke<input name="weaponES" value="${escapeHtml(w.effectiveStrength)}"></label><label>Lade-/Feuerzeit<input name="weaponLoad" value="${escapeHtml(w.load)}"></label><label>Reichweite kurz<input name="rangeShort" value="${escapeHtml(w.rangeShort)}"></label><label>Reichweite lang<input name="rangeLong" value="${escapeHtml(w.rangeLong)}"></label><label>Reichweite extrem<input name="rangeExtreme" value="${escapeHtml(w.rangeExtreme)}"></label></div></section>
      <section id="item-effects-fields" class="nested-fields item-effects-fields"><div class="section-title"><div><span class="eyebrow">Besondere Regeln</span><h3>Effekte &amp; Fähigkeiten</h3></div><button type="button" class="mini-button" id="add-item-effect">＋ Effekt</button></div><p class="hint">Hier kannst du einer Waffe oder Rüstung mehrere Effekte zuweisen, z. B. Armour Piercing oder Warp Attack. Die Regelwirkung wird nicht automatisch erfunden. Optional eingetragene Profilboni werden jedoch automatisch verrechnet und erscheinen unter Kampf → Magische Effekte, sobald der Gegenstand aktiv angelegt/griffbereit ist.</p><div class="item-effects-editor" id="item-effects-list">${(item.effects||[]).map(itemEffectEditorRow).join("")}</div></section>
      <label>Beschreibung &amp; Regeln<textarea name="description" rows="5" placeholder="Beschreibung, Anwendung, Sonderregeln oder andere wichtige Hinweise zum Gegenstand">${escapeHtml(item.description)}</textarea><small>Dieser Text erscheint in der Gegenstandsliste als ausklappbarer Bereich „Beschreibung &amp; Regeln“.</small></label>
      <label>Kurze Notiz<textarea name="note">${escapeHtml(item.note)}</textarea></label><button class="full-button">Speichern</button></form>`);

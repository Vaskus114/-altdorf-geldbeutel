(() => {
  ...
  const itemDescriptionDetails = (item,extraClass="") => {
    ...
    if(!text)return "";
    return `<details class="rule-description item-rule-description ${extraClass}"><summary>Beschreibung &amp; Regeln</summary><p>${escapeHtml(text)}</p></details>`;
  };
  ...
  const itemEditor = (locationId,itemId,previous,returnContext={locationId}) => {
    ...
    const toggleFields=()=>{$("#armor-fields",form).classList.toggle("hidden",typeSelect.value!=="armor");$("#weapon-fields",form).classList.toggle("hidden",typeSelect.value!=="weapon");$("#item-effects-fields",form).classList.toggle("hidden",!["armor","weapon"].includes(typeSelect.value))};

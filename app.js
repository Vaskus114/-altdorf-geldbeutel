(() => {
  ...
  const combatTab = character => {
    ...
    const spellEffects=activeSpells.map(spell=>`<article class="active-effect-card spell-effect"><div class="active-effect-head"><span class="effect-kind">Zauber</span><strong>${escapeHtml(spell.name)}</strong></div>${spell.armourBonus?`<p><b>Rüstungswirkung:</b> ${signed(spell.armourBonus)} AP auf alle Trefferzonen.</p>`:""}${spell.description?`<p>${escapeHtml(spell.description)}</p>`:""}</article>`).join("");
    const equipmentEffects=activeItemEffects.map(({item,effect})=>`<article class="active-effect-card item-effect"><div class="active-effect-head"><span class="effect-kind">${escapeHtml(effectKindLabel(effect.kind))}</span><strong>${escapeHtml(effect.name)}</strong></div><small>Quelle: ${escapeHtml(item.name)}</small>${effect.description?`<p>${escapeHtml(effect.description)}</p>`:`<p class="hint">Keine Beschreibung hinterlegt.</p>`}</article>`).join("");
  ...
  const itemEditor = (locationId,itemId,previous,returnContext={locationId}) => {
    ...
    const layer=modal(`<form class="sheet form equipment-form"><div class="handle"></div><div class="heading"><div><span class="eyebrow">Inventar &amp; Charakterbogen</span><h2>${existing?"Gegenstand bearbeiten":"Gegenstand hinzufügen"}</h2></div><button type="button" class="close">×</button></div><label>Vorlage<select name="presetId">${presetOptions(item.presetId)}</select></label><div class="form-grid two"><label>Name<input name="name" required value="${escapeHtml(item.name)}" placeholder="z. B. Kettenmantel"></label><label>Art<select name="type"><option value="generic" ${item.type==="generic"?"selected":""}>Allgemein</option><option value="armor" ${item.type==="armor"?"selected":""}>Rüstung / Schild</option><option value="weapon" ${item.type==="weapon"?"selected":""}>Waffe</option></select></label><label>Anzahl<input name="quantity" type="number" min="1" max="9999" value="${item.quantity}"></label><label>ENC je Stück<input name="enc" type="number" min="0" max="99999" step="1" value="${item.enc}"></label></div><span class="eyebrow">Wert je Stück</span>${moneyFields(item.value)}${locationId==="body"?`<label class="check-line"><input name="equipped" type="checkbox" ${item.equipped?"checked":""}><span>Aktiv angelegt / griffbereit</span></label>`:""}
      ...
      <section id="item-effects-fields" class="nested-fields item-effects-fields"><div class="section-title"><div><span class="eyebrow">Besondere Regeln</span><h3>Effekte &amp; Fähigkeiten</h3></div><button type="button" class="mini-button" id="add-item-effect">＋ Effekt</button></div><p class="hint">Hier kannst du einer Waffe oder Rüstung mehrere Effekte zuweisen, z. B. Armour Piercing oder Warp Attack. Die Wirkung wird nicht automatisch erfunden oder verrechnet; sie erscheint als Regelhinweis unter Kampf → Magische Effekte, sobald der Gegenstand aktiv angelegt/griffbereit ist.</p><div class="item-effects-editor" id="item-effects-list">${(item.effects||[]).map(itemEffectEditorRow).join("")}</div></section>
